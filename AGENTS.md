...
> - **Custom cursor + ripple interaction layer (自定义指针 + 涟漪)**: The native OS pointer is replaced site-wide with the Game-Icon-Pack `cursor` glyph, and mouse move/click spawn MD3-tinted ripples. Logic is isolated in the **state/logic hook** `hooks/use-pointer-fx.ts` (capability probe via `matchMedia`, pointer-coordinate smoothing with `framer-motion`'s `useSpring`, distance+time-throttled ripple generation, and toggling the `cursor-fx-active` class on `<html>`); presentation is the **`"use client"` leaf** `components/ui/cursor-fx.tsx`, mounted **once** in `layout.tsx` inside `ThemeProvider` (root stays a pure-orchestration RSC, §1.6). The pointer reuses `<Icon name="cursor" />` tinted via the `text-brand-primary` MD3 token so it **recolors with the theme** (§1.8); the arrow hotspot is corrected against the 10×10 viewBox so the click point matches the visual tip. Ripples are styled by `globals.css` (`.cursor-ripple` + `--move`/`--click` modifiers + the `@keyframes aki-cursor-ripple`, all colored via `color-mix()` over MD3 tokens — move=tertiary thin ring, click=secondary filled bloom) and self-remove on `onAnimationEnd`. The whole layer is `aria-hidden` + `pointer-events-none` and **only activates on fine pointers** (`@media (pointer: fine)`); touch devices fall back to native interaction and `prefers-reduced-motion` suppresses ripples. The only JS-dynamic inline styles are the pointer transform and ripple `left/top` — the explicitly allowed positioning exception of §1.3. **This feature adds no new npm dependency** (reuses `framer-motion` + the vendored icon system).
> - **Installed dependencies**: `clsx` + `tailwind-merge` (mandated `cn()`, §1.3); `gray-matter` + `marked` (Markdown frontmatter + rendering); `next-themes` (class-based dark mode); `framer-motion` (staggered/fade-up animations + the custom-cursor spring). Icons are **not** an npm dependency — see the icon-system note above.
