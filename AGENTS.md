...
│   │   ├── layout/                   # Structural layout components (Sidebar, Header, NavLinks, CategoryNav, Breadcrumbs)
...
│   │   ├── navigation.ts             # Nav items (supports one-level children submenu) + category matrix (collapsible submenu → /categories routes) + label map + CATEGORIES_BASE_PATH/categoryHref helpers
...
│   │   ├── mdx.ts                    # Markdown parsing/rendering (gray-matter + marked) → typed posts (+ slug→title map)
...
> - **Navigation supports one level of submenu**: `NavItem` carries an optional `children?: readonly NavItem[]`. The «项目 / 作品» (Projects/Works) top-level item links to `/projects` and derives its three children directly from `data/projects.ts` (single source of truth — menu items never drift from the actual pages). `components/layout/nav-links.tsx` (the client leaf) renders parents with an expand/collapse toggle (`aria-expanded`/`aria-controls`, auto-expanded when the current route is inside the section); the submenu is **desktop-only** (mobile bottom-bar taps the parent straight to `/projects`). The project pages are pure controllers reading `data/projects.ts` and orchestrating `components/projects/*`; `/projects/[category]` is SSG via `generateStaticParams` over the category keys (`blog-source` / `personal` / `oss`), which double as the route slugs.
> - **Category routes & submenu (文章分类独立路由 + 二级菜单)**: Article categories are **standalone routes, decoupled from the home feed** — there is no `/?category=<key>` query param anymore. `data/navigation.ts` is the single source of truth: it exports the `categoryItems` matrix (each with a `summary`), `CATEGORIES_BASE_PATH` (`/categories`), `CATEGORIES_LABEL`, and a `categoryHref(key)` helper. `app/categories/page.tsx` (RSC) is an overview of category cards with per-category post counts; `app/categories/[category]/page.tsx` (RSC) is SSG via `generateStaticParams` over the category keys (`tech`/`anime`/`life`/`notes`, which double as the route slugs) + per-category `generateMetadata`, server-side filtering `getAllPosts()` to that category and handing the list to `components/blog/post-list.tsx`; an invalid slug triggers `notFound()`. The sidebar's category section (`components/layout/category-nav.tsx`, the client leaf) is a **collapsible second-level menu** mirroring the Projects menu: the «文章分类» parent is a `Link` to `/categories` plus a separate expand/collapse toggle (`aria-expanded`/`aria-controls` + rotated `chevron-down`), and each child links to `categoryHref(key)`. Active state is now derived from `usePathname` (path-based), so the leaf **no longer needs a `<Suspense>` boundary** in `sidebar.tsx`. The submenu is **desktop-only** (`hidden md:flex`), defaults to open for discoverability, and the mobile bottom-bar omits it to save space.
> - **Header breadcrumbs (顶部页眉面包屑)**: The glass `<header>` (`components/layout/header.tsx`, RSC) renders a breadcrumb trail on desktop. `lib/breadcrumbs.ts` exposes a pure `buildBreadcrumbs(pathname, postTitles)` that derives the trail labels/links **from the data layer** — top-level labels reuse `navItems` (so «项目 / 作品», «归档»… never drift) plus `CATEGORIES_BASE_PATH`→`CATEGORIES_LABEL` for the category overview, project sub-categories resolve through `projectCategoryMap`, category sub-pages (`/categories/<key>`) resolve through `categoryLabelMap`, and `/posts/<slug>` resolves the slug to its Chinese title via the `slug→title` map from `getPostTitleMap()` in `lib/mdx.ts` (`/posts` itself is a non-navigable label since there is no post-index route). `components/layout/breadcrumbs.tsx` is the **client leaf** (`usePathname`) and renders strict semantic markup (`<nav aria-label="面包屑"><ol><li>`, `aria-current="page"` on the last crumb, a rotated `chevron-down` icon as the separator). On the home route (only «首页») it falls back to the site tagline so the header stays balanced; the breadcrumb is desktop-only (`hidden md:block`), the mobile header keeps the brand link.
