...
> - **Custom cursor + ripple interaction layer (自定义指针 + 涟漪)**: The native OS pointer is replaced site-wide with the Game-Icon-Pack `cursor` glyph, and mouse move/click spawn MD3-tinted ripples. Logic is isolated in the **state/logic hook** `hooks/use-pointer-fx.ts` (capability probe via `matchMedia`; a `requestAnimationFrame` loop that exponentially eases the pointer toward the target and writes `transform` **directly to the DOM**; distance+time-throttled ripple spawning via **imperative `appendChild`** with self-removal on `animationend`; and toggling the `cursor-fx-active` class on `<html>`). **Why rAF + imperative DOM instead of React state / Framer Motion**: pointer movement is extremely high-frequency, so driving it through `setState` or a soft spring caused visible lag and jank — the imperative approach does **zero React re-renders per move**, eliminating the latency and guaranteeing move ripples appear (§1.5 performance-first). Presentation is the **`"use client"` leaf** `components/ui/cursor-fx.tsx`, which only renders a static skeleton (a layer container + the cursor element) and hands its refs to the hook; it is mounted **once** in `layout.tsx` inside `ThemeProvider` (root stays a pure-orchestration RSC, §1.6). The pointer reuses `<Icon name="cursor" />` tinted via the `text-brand-primary` MD3 token so it **recolors with the theme** (§1.8); the arrow hotspot is corrected against the 10×10 viewBox so the click point matches the visual tip. Ripples are styled by `globals.css` (`.cursor-ripple` + `--move`/`--click` modifiers + `@keyframes aki-cursor-ripple` — move=tertiary thin ring via `color-mix()` over MD3 tokens, click=a primary-blue→white radial-gradient bloom). The whole layer is `aria-hidden` + `pointer-events-none` and **only activates on fine pointers** (`@media (pointer: fine)`); touch devices fall back to native interaction and `prefers-reduced-motion` suppresses ripples. The only JS-dynamic inline styles are the pointer transform and ripple `left/top` — the explicitly allowed positioning exception of §1.3. **This feature adds no new npm dependency.**
