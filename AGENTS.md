...
│   │   └── use-pointer-fx.ts         # Custom-cursor follow + move/click ripple logic (device probe, spring, throttle)
...
> - **Custom cursor + ripple interaction layer (自定义指针 + 涟漪)**: The native OS pointer is replaced site-wide with the Game-Icon-Pack `cursor` glyph, and mouse move/click spawn MD3-tinted ripples. Logic is isolated in the **state/logic hook** `hooks/use-pointer-fx.ts` (capability probe via `matchMedia`; on each `pointermove` it writes the pointer `transform` **directly to the DOM at the real pointer position**; distance+time-throttled ripple spawning via **imperative `appendChild`** with self-removal on `animationend`; and toggling the `cursor-fx-active` class on `<html>`). **Why imperative direct-DOM instead of React state / Framer Motion / eased following**: pointer movement is extremely high-frequency, so driving it through `setState` or a soft spring caused re-render jank, and **any** smoothing/easing makes the cursor only chase a fraction of the gap per frame so it visibly trails a fast mouse — the cursor is therefore snapped exactly onto the pointer every move (transform-only, compositor, no reflow → zero lag), with **zero React re-renders per move** (§1.5 performance-first). Presentation is the **`"use client"` leaf** `components/ui/cursor-fx.tsx`, which only renders a static skeleton (a layer container + the cursor element) and hands its refs to the hook; it is mounted **once** in `layout.tsx` inside `ThemeProvider` (root stays a pure-orchestration RSC, §1.6). The pointer reuses `<Icon name="cursor" />` tinted via the `text-brand-primary` MD3 token so it **recolors with the theme** (§1.8); the arrow hotspot is corrected against the 10×10 viewBox so the click point matches the visual tip. Ripples are styled by `globals.css` (`.cursor-ripple` + `--move`/`--click` modifiers + `@keyframes aki-cursor-ripple` — move=tertiary ring+soft fill via `color-mix()` over MD3 tokens, click=a primary-blue→secondary-pink radial-gradient bloom — all colors are MD3 tokens, no hardcoded hex). The whole layer is `aria-hidden` + `pointer-events-none` and **only activates on fine pointers** (`@media (pointer: fine)`); touch devices fall back to native interaction and `prefers-reduced-motion` suppresses ripples. The only JS-dynamic inline styles are the pointer transform and ripple `left/top` — the explicitly allowed positioning exception of §1.3. **This feature adds no new npm dependency.**
