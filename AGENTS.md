...
│   │   ├── navigation.ts             # Nav items (supports one-level children submenu) + category matrix (rendered as a collapsible submenu) + label map
...
│   │   ├── breadcrumbs.ts            # Pure builder: pathname → breadcrumb trail (derived from nav + projects data)
