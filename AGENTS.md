...
│   │   ├── not-found.tsx             # Custom 404 (RSC) — renders inside root layout, no opaque fill so the -z-10 立绘 background shows through
│   │   ├── error.tsx                 # Custom error boundary (CC, required) — same rationale; offers reset()/返回首页
...
│   │   └── site-config.ts            # Site brand/SEO metadata + author info + social links + heroBackground (focal-point config system)
