...
│   │   ├── layout/                   # Structural layout components (Sidebar, Header, NavLinks, Breadcrumbs)
│   │   ├── providers/                # Client context providers (ThemeProvider wrapping next-themes)
│   │   └── ui/                       # Atomic base components (Badge, Icon, ThemeToggle, Reveal, Avatar, CursorFx)
...
│   │   └── use-pointer-fx.ts         # Native-cursor toggle + move/click ripple logic (device probe, throttle, imperative appendChild)
│   ├── lib/                          # 【Core Service Layer】 Utility functions and parsers
│   │   ├── mdx.ts                    # Markdown parsing/rendering (gray-matter + marked) → typed posts (+ slug→title / slug→category maps)
...
> - **Custom cursor + ripple interaction layer (自定义指针 + 涟漪)**: The native OS pointer is replaced site-wide with the Game-Icon-Pack cursor arrow, and mouse move/click spawn MD3-tinted ripples. **Why the pointer is a native `cursor:url()` rather than a DOM element that follows the mouse**: any DOM-simulated cursor must traverse the «pointer-event → JS → browser composite» pipeline and therefore trails the real pointer by at least ~1 frame (visibly so on fast moves — the user-perceived "can't keep up"), whereas a native CSS cursor is drawn directly by the OS/GPU with **zero latency, perfectly locked to the hardware pointer**. So the arrow itself is shipped as two pre-generated, pre-colored assets in `public/cursors/` (derived from Game-Icon-Pack `3.Editing Tools/cursor-default.svg`) and applied in `globals.css` via `cursor: url(...) 9 5, auto !important` under `@media (pointer: fine)`, with a `html.dark.cursor-fx-active` override swapping to the dark asset so it **recolors with the theme** (`9 5` is the arrow-tip hotspot, the 10×10 viewBox mapped onto the 28px render, so the click point matches the visual tip). **The CSS references the `.png` files, not the `.svg`**: Chrome's SVG-cursor support is buggy (it frequently renders an SVG cursor as blank, so the pointer literally *disappears*), whereas PNG cursors are stable in Chrome/Firefox/Safari. The `.svg` files are kept only as the human-editable, MD3-token-colored masters; the actually-used `cursor-{light,dark}.png` are exported from them (28×28) via `node -e "const s=require('sharp');['light','dark'].forEach(v=>s('public/cursors/cursor-'+v+'.svg',{density:320}).resize(28,28).png().toFile('public/cursors/cursor-'+v+'.png'))"`. Each master's fill mirrors that mode's `--md-sys-color-primary` (light `#0061a4` / dark `#9ecaff`) — a baked-in hex is unavoidable for a native cursor image (an exception to §1.8); **if the primary token changes, re-color the two SVG masters and re-export the PNGs**. Logic is isolated in the **state/logic hook** `hooks/use-pointer-fx.ts` (capability probe via `matchMedia`; toggles the `cursor-fx-active` class on `<html>` to activate the native arrow; distance+time-throttled move ripples + click ripples spawned via **imperative `appendChild`** with self-removal on `animationend` — **zero React re-renders**, §1.5). Presentation is the **`"use client"` leaf** `components/ui/cursor-fx.tsx`, which renders only a static ripple-layer skeleton and hands its ref to the hook; it is mounted **once** in `layout.tsx` inside `ThemeProvider` (root stays a pure-orchestration RSC, §1.6). Ripples are styled by `globals.css` (`.cursor-ripple` + `--move`/`--click` modifiers + `@keyframes aki-cursor-ripple` — move=tertiary ring+soft fill via `color-mix()` over MD3 tokens, click=a primary-blue→secondary-pink radial-gradient bloom — all colors are MD3 tokens, no hardcoded hex). The whole layer is `aria-hidden` + `pointer-events-none` and **only activates on fine pointers** (`@media (pointer: fine)`); touch devices fall back to native interaction and `prefers-reduced-motion` suppresses ripples. The only JS-dynamic inline styles are the ripple `left/top` — the explicitly allowed positioning exception of §1.3. **This feature adds no new npm dependency.**
