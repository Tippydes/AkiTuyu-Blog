...
│   │   ├── navigation.ts             # Nav items (optional href for group-only parents + one-level children submenu + desktopOnly) — «文章分类» & «项目/作品» derive children here; + category matrix + label map
...
> - **Navigation submenus (one level, data-driven)**: `NavItem` carries an optional `children?: readonly NavItem[]`, an **optional `href`** (group-only parents with no standalone page omit it), and an optional `desktopOnly` flag. `components/layout/nav-links.tsx` (the client leaf) renders each top-level entry in three forms: (1) no children → plain link; (2) children + `href` («项目 / 作品» → `/projects`) → parent link plus a separate expand/collapse toggle button; (3) children **without** `href` («文章分类») → the whole row is the toggle (no navigation), styled identically to a top-level link. Both submenus live in the **same** main `<nav>` list (single column) and derive children from a single source: «项目 / 作品» from `data/projects.ts` (`/projects/[category]` is SSG over `blog-source`/`personal`/`oss`), «文章分类» from the `categoryItems` matrix (children link to the home filter `/?category=<key>`). Submenus are **desktop-only**; the «文章分类» entry is additionally `desktopOnly` so it never reaches the mobile bottom-bar (a group-only toggle has no mobile landing). The active category is highlighted by reading the `category` search param via `useSearchParams`; because that opts a subtree out of static prerender, `NavLinks` wraps its reader in a `<Suspense>` whose fallback is a `usePathname`-only variant (no query → prerenderable), so `/about`/`/archive` keep their nav in the static HTML and only the category highlight fills in after hydration.
